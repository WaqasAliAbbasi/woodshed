...
- `SequenceMatcher` (Notes mode) — order-driven, no clock. Chords are
  ...
  chord is also all-or-nothing: a wrong note part-way through one — or a
  gap longer than `CHORD_WINDOW_MS` since its first note — discards the
  notes already played in it, so a grand-staff chord can't be completed one
  hand at a time. That window is the one place time enters this matcher, and
  only to define "together"; nothing here is scored early or late. A
